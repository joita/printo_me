...
var dgUI = {
	...
	product:{
		...
		design: function(e, position){
			...
			var input = jQuery(e).parent().parent().data('color');
			var number = jQuery(e).parent().parent().attr('id').replace('color_', '');
			jQuery.ajax({
				type: "POST",
				url: base_url + "/administracion/productos/diseno",
				data: { position: position, number: number, title: input }
		...
		setup: function(position, number){
			...
			if(items.length <  4){
				var html = '',
					...
					html = html + '<i class="fa fa-clipboard"></i> &nbsp; ';
					html = html + '<span>Área Imprimible</span>';
					html = html + '<div class="layer-action pull-right">';
					html = html + '<a title="" href="javascript:void(0)"><i class="fa fa-arrows"></i></a>';					
					html = html + '</div>';
			...
			jQuery.each(items, function(i, item){
				...
				if(item.id == 'area-design'){
					...
					html = html + '<img width="10" height="10" alt="" src="' +url +'/'+ item.img+ '" /> &nbsp;';
					html = html + '<span>' + item.id + '</span>';
				}
				html = html + '<div class="layer-action pull-right">';
				html = html + 	'<a title="" href="javascript:void(0)"><i class="fa fa-arrows"></i></a>&nbsp;';
				html = html + 	'<a title="" onclick="dgUI.product.layers.remove(this)" href="javascript:void(0)"><i class="fa fa-trash-o"></i></a>';
				html = html + '</div>';
		...
		save: function(position, color){
			...
			jQuery('#layers .layer').each(function(){
				...
				if(id == 'area-design')
				{
					...
					var media_pos = src.indexOf("media");
					var src_len = (src.length)-1;
					src = src.substr(media_pos, src_len);
			...
			jQuery('#ajax-modal').foundation('reveal', 'close');
		},
		...
		addDesign: function(images){
			if(images.length > 0)
			{
				...
				for(i=0; i<images.length; i++)
				{
					...
					html = html + 		'<i class="fa fa-arrows"></i>';
					html = html + 	'</a>';
