...
class Tienda_Carrito extends MY_Controller {
    ...
    public function index($nombre_tienda_slug = null) {
        ...
        if($this->cart->total_items() > 0) {
            $this->load->view('tienda/carrito/despliegue');
        } else {
            $this->load->view('tienda/carrito/vacio');
        }
    ...
    */
	...
	public function index($nombre_tienda_slug = null) {
		if (strpos(uri_string(), 'Servicios') !== false && strpos(uri_string(), 'carrito/vacio') === false && $this->cart->total_items() == 0) {
			redirect('tienda/'.$nombre_tienda_slug.'/carrito/vacio');
		}
		...
		switch($dia) {
			case 1:
				$informacion['recibir'] = date("Y-m-d H:i:s", strtotime("+11 days"));
			break;

			case 2:
				$informacion['recibir'] = date("Y-m-d H:i:s", strtotime("+13 days"));
			break;

			case 3:
				$informacion['recibir'] = date("Y-m-d H:i:s", strtotime("+14 days"));
			break;
			...
			case 7:
				$informacion['recibir'] = date("Y-m-d H:i:s", strtotime("+12 days"));
			break;
		}
		...
		if($this->cart->total_items() > 0) {
			$this->load->view('tienda/carrito/despliegue', $informacion);
		} else {
			$this->load->view('tienda/carrito/vacio');
		}
	...
	public function cupon() {
		...
		echo json_encode(array('status' => $this->cart->add_coupon($name)));
	}
	...
	public function generar_link_paypal($nombre_tienda_slug = null)
	{
		...
		$redirectUrls->setReturnUrl(site_url('tienda/'.$nombre_tienda_slug.'/carrito/terminar_paypal'))
					 ->setCancelUrl(site_url('tienda/'.$nombre_tienda_slug.'/carrito/pagar'));
	...
	public function terminar_paypal($nombre_tienda_slug = null)
	{
		if(!$this->input->get('paymentId') && !$this->input->get('token') && !$this->input->get('PayerID')) {
			redirect('tienda/'.$nombre_tienda_slug.'/carrito/error-pago');
		} else {
			...
			try {
				...
				redirect('tienda/'.$nombre_tienda_slug.'/carrito/pedido-completado-paypal');
	...
	public function pagar($nombre_tienda_slug = null) {
		...
		try {
			$datos_header['link_paypal'] = $this->generar_link_paypal($nombre_tienda_slug);
		} catch (Exception $ex) {
			//$ex->getMessage();
			//redirect('tienda/'.$nombre_tienda_slug.'/carrito');
			//print_r($ex);
		...
		$footer_datos['scripts'] = 'tienda/carrito/script_pago';

		$this->load->view('tienda/header', $datos_header);

		if($this->cart->total_items() > 0) {
			$this->load->view('tienda/carrito/pago');
		} else {
			redirect('tienda/'.$nombre_tienda_slug.'/carrito');
		}
		$this->load->view('tienda/footer', $footer_datos);
	}

	public function terminar($nombre_tienda_slug = null) {
		...
		if($forma_pago == 'tdc') {
			try {
				...
				$email_compra->addTo($datos_correo->email, $datos_correo->nombre)
							 ...
							 ->setHtml($this->load->view('plantillas_correos/confirmacion_tarjeta', $datos_correo, TRUE))
				;
				...
				$email_administracion->addTo('administracion@printome.mx', 'Administración Printome')
									 ...
									 ->setHtml($this->load->view('plantillas_correos/confirmacion_compra_admin', $datos_correo, TRUE))
									 ->addAttachment('assets/pdf/'.$this->pdf_pedido_archivo($id_pedido), 'pedido_printome_'.$datos_correo->numero_pedido.'.pdf')
				;
				$sendgrid->send($email_administracion);
				...
				redirect('tienda/'.$nombre_tienda_slug.'/carrito/pedido-completado-tarjeta');
			...
			try {
				...
				$email_oxxo->addTo($datos_correo->email, $datos_correo->nombre)
						   ...
						   ->setHtml($this->load->view('plantillas_correos/aviso_oxxo', $datos_correo, TRUE))
				...
				$email_omar->addTo('administracion@printome.mx', 'Administración Printome')
						   ...
						   ->setHtml($this->load->view('plantillas_correos/fyi_omar_oxxo', $datos_correo, TRUE))
				;
				...
				redirect('tienda/'.$nombre_tienda_slug.'/carrito/pedido-completado-oxxo');
			} catch (Conekta_Error $e) {
				if($this->session->has_userdata('correo_temporal')) {
					ac_quitar_etiqueta($this->session->correo_temporal, 'pedido-completado');
				} else {
	...
	public function pedido_completado($metodo_pago, $nombre_tienda_slug = null) {
		...
		$this->load->view('tienda/header', $datos_header);
		$this->load->view('tienda/carrito/gracias', $barcode);
		$this->load->view('tienda/footer');
	}

	public function pdf_oxxo($id_pedido, $nombre_tienda_slug = null){
	...
	public function pdf_oxxo_archivo($id_pedido, $nombre_tienda_slug = null){
		$this->load->helper(array('dompdf', 'file'));
	...
	public function pdf_pedido_archivo($id_pedido, $nombre_tienda_slug = null){
		$this->load->helper(array('dompdf', 'file'));
	...
	public function pdf_pedido($id_pedido, $nombre_tienda_slug = null){
		$this->load->helper(array('dompdf', 'file'));
		$contenido['accion'] = 'despliegue';
		$contenido['pedidos'] = 'despliegue';
		$contenido['id_pedido'] = $id_pedido;
  ...
  public function abrir_dhl($pedido_id) {
		...
		foreach($productos as $indice=>$data) {
			...
			if($indice == 0) {
				...
				if(isset($dibujo_front)) {
                    ...
                    $design['images']['dis_front'] = $filename_dis_front;
				} else {
					$img_front = new \Imagick();
					$img_front->readImage(realpath($design_info->front[$mi_indice]->{1}->img));
					$img_front->resizeimage(500, 500, \Imagick::FILTER_LANCZOS, 1);
				...
				if(isset($dibujo_back)) {
                    ...
                    $design['images']['dis_back'] = $filename_dis_back;
				} else {
					$img_back = new \Imagick();
					$img_back->readImage(realpath($design_info->back[$mi_indice]->{1}->img));
					$img_back->resizeimage(500, 500, \Imagick::FILTER_LANCZOS, 1);
				...
				if(isset($dibujo_left)) {
                    ...
                    $design['images']['dis_left'] = $filename_left;
				} else {
					// Imagen left
					$img_left = new \Imagick();
				...
				if(isset($dibujo_right)) {
                    ...
                    $design['images']['dis_right'] = $filename_dis_right;
				} else {
					// Imagen right
					$img_right = new \Imagick();
					$img_right->readImage(realpath($design_info->right[$mi_indice]->{1}->img));
	...
	public function obtener_custom_especifico()
	{
		...
		if(sizeof($product_ids) == 1) {
			...
			<div class="row collapse estilo_completo" data-did="'.$el_diseno['design_id'].'">
				...
				<div class="small-18 medium-15 large-14 columns">
					<div class="row collapse">
						<div class="medium-11 columns">
							...
							<div class="clearfix" id="tallitas">';
			...
			foreach($product_ids as $id) {
				foreach($cart_contents as $item) {
					$options = $item['options'];
					if(isset($options['id_diseno'])) {
						if($item['id'] == $id && $item['options']['id_diseno']) {
							$cantidades_array[$id] = array();
						}
					}
				}
			}
			...
			foreach($product_ids as $id) {
				...
				foreach($cart_contents as $item) {
					...
					$options = $item['options'];
					if(isset($options['id_diseno'])) {
						if($options['id_diseno'] == $id_diseno && $item['id'] == $id) {
							$cantidades_array[$id][$options['sku']] = array(
								'talla' => $options['sku'],
								'cantidad' => $item['qty']
							);
						}
			...
			foreach($customs as $indice=>$custom) {
				foreach($cart_contents as $item) {
					...
					if(isset($options['id_diseno'])) {
						if($options['id_diseno'] == $custom['design_id']) {
							$customs[$indice]['quantity'] += $item['qty'];
							$customs[$indice]['sizes'] = $cantidades_array[$custom['product_id']];
						}
					}
				}
			...
			?>
			<div class="row collapse estilo_completo" data-did="<?php echo $custom['design_id']; ?>" data-id_producto="<?php echo $custom['product_id']; ?>" data-color="<?php echo $custom['colors'][0]; ?>">
				<div class="small-18 medium-3 large-4 columns">
	...
	public function reinsertar_custom_multiple_en_carrito() {
		...
		foreach($customs as $indice=>$custom) {
			...
			$color = $custom['colors']['0'];
			foreach($this->input->post('estilos') as $estilo) {
				if(($estilo['id_producto'] == $id_producto) && ($estilo['color'] == $color)) {
	...
	public function borrar_custom_en_carrito($id_fila, $id_diseno, $nombre_tienda_slug = '')
	{
		$data = array(
			'rowid' => $id_fila,
			'qty'   => 0
	...
	public function error($nombre_tienda_slug = null) {
		// Config
		$datos_header['seccion_activa'] = 'Servicios';
		...
		$this->load->view('tienda/header', $datos_header);
		$this->load->view('tienda/carrito/error', array());
		$this->load->view('tienda/footer');
	}
