...
class Cargos extends MY_Admin
{
    ...
    public function desplegar_cargos(){
        ...
        foreach($cargos as $cargo){
            ...
            if($cargo->estatus_pago == 'pending_payment' || $cargo->estatus_pago == 'por_realizar'){
                $inner_info->eliminar .= "<div class='text-center eliminar'><button class='btn btn-danger btn-eliminar' onclick='borrarCargo(".$cargo->id_cargo.",".$cargo->id_pedido.")'><i class='fa fa-trash'></i></button> 
                                        <input type='hidden' value='".$cargo->id_cargo."'></div>";
            }else{
                $inner_info->eliminar .= "<div class='text-center eliminar'><button class='btn btn-danger disabled btn-eliminar'><i class='fa fa-trash'></i></button> </div>";
            }
    ...
    public function delete_cargo(){
        ...
        $cargo_estatus = new stdClass();
        $cargo_estatus->estatus = 0;


        $this->db->where('id_cargo', $cargo);
        $this->db->update('CargosExtra', $cargo_estatus);
        $estatus  = 1;
        return $estatus;
    }
    public function consultar_cargo(){
        ...
        print_r($pedido_id);
        print_r($pedido_id->id_pedido);
        print_r($pedido_id->{'id_pedido'});
        print_r('total '.$total_resultado);*/
        return $pedido;
    }
    public function existe_shopify(){
        $num_pedido = $this->input->post('num_pedido');
        $pedido_id = $this->cargos_extra_m->existe_shopify($num_pedido);
