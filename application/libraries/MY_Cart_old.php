...
class MY_Cart extends CI_Cart {
	...
	public function obtener_saldo_a_favor()
	{
		if (!is_null($this->CI->session->login['id_cliente'])) {
			$info = $this->CI->db->get_where('Clientes', array('id_cliente' => $this->CI->session->login['id_cliente']))->row();
	...
	public function add_coupon($name = NULL)
	{
		...
		if ($query->num_rows() > 0) {
            ...
            if (($cupon_vigente > 0 && $cantidad_restante) && $cupon && $flag_producto && $flag_usuario &&($this->obtener_subtotal() > $row->monto_minimo) && ($row->id_cliente != $this->CI->session->login['id_cliente'])) {
            ...
            if (($cupon_vigente > 0 && $cantidad_restante) && $cupon_usado && ($this->obtener_subtotal() > $cupon->monto_minimo)) {
                if($cupon->tipo != '4') {
                    if ($this->CI->session->descuento_global->descuento > 0 && $this->CI->session->descuento_global->descuento < 1 && $cupon->tipo != 3) {
                        ...
                        return ($this->obtener_subtotal() - $this->obtener_saldo_a_favor()) * (1 - $this->CI->session->descuento_global->descuento) + $this->obtener_costo_envio();
                    } else {
	...
	public function obtener_costo_envio() {
        ...
        if ($test) {
            ...
            if ($CI->session->recoger == 'gratis') {
                    ...
                    foreach ($campanas as $id_enhance) {
                        ...
                        if ($productos_campana > 0) {
                            if ($productos_campana <= 25) {
                                $costos_separados_campanas[$id_enhance] = 99;
                            } else {
                                $costos_separados_campanas[$id_enhance] = 99 + (($productos_campana - 25) * 0);
                            }
                        } else {
                ...
                if ($customs > 0) {
                    if ($customs <= 25) {
                        $envios_por_pagar['customs'] = 99;
                    } else {
                        $envios_por_pagar['customs'] = 99 + (($customs - 25) * 0);
                    }
                } else {
